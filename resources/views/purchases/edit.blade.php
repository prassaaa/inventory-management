    ...
    <div class="d-flex align-items-center justify-content-between mb-4">
        <div>
            <h1 class="h3 mb-0 text-dark fw-bold">
                <i class="fas fa-edit me-2 text-primary"></i> Edit Pembelian
            </h1>
            <p class="text-muted">Edit transaksi pembelian: <span class="fw-medium">{{ $purchase->invoice_number }}</span></p>
        </div>
    ...
    <div class="card shadow mb-4">
        ...
        <div class="card-body">
            <form action="{{ route('purchases.update', $purchase) }}" method="POST" id="purchase-form">
                ...
                <div class="row">
                    <div class="col-md-6">
                        <div class="form-group mb-3">
                            ...
                            <input type="text" class="form-control @error('invoice_number') is-invalid @enderror" id="invoice_number" name="invoice_number" value="{{ old('invoice_number', $purchase->invoice_number) }}" required>
                        ...
                        <div class="form-group mb-3">
                            ...
                            <input type="date" class="form-control @error('date') is-invalid @enderror" id="date" name="date" value="{{ old('date', $purchase->date->format('Y-m-d')) }}" required>
                            @error('date')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                        ...
                        <div class="form-group mb-3">
                            ...
                            <select class="form-select select2 @error('supplier_id') is-invalid @enderror" id="supplier_id" name="supplier_id" required>
                                ...
                                @foreach($suppliers as $supplier)
                                    <option value="{{ $supplier->id }}" data-payment-term="{{ $supplier->payment_term }}" {{ old('supplier_id', $purchase->supplier_id) == $supplier->id ? 'selected' : '' }}>
                                        {{ $supplier->name }}
                                    </option>
                                @endforeach
                            </select>
                    ...
                    <div class="col-md-6">
                        <div class="form-group mb-3">
                            ...
                            <select class="form-select @error('payment_type') is-invalid @enderror" id="payment_type" name="payment_type" required>
                                <option value="tunai" {{ old('payment_type', $purchase->payment_type) === 'tunai' ? 'selected' : '' }}>Tunai</option>
                                <option value="tempo" {{ old('payment_type', $purchase->payment_type) === 'tempo' ? 'selected' : '' }}>Tempo (Kredit)</option>
                            </select>
                            @error('payment_type')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        ...
                        <div class="form-group mb-3 due-date-group {{ old('payment_type', $purchase->payment_type) === 'tempo' ? '' : 'd-none' }}">
                            <label for="due_date" class="form-label">Tanggal Jatuh Tempo <span class="text-danger">*</span></label>
                            <input type="date" class="form-control @error('due_date') is-invalid @enderror" id="due_date" name="due_date" value="{{ old('due_date', $purchase->due_date ? $purchase->due_date->format('Y-m-d') : '') }}">
                            @error('due_date')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                        ...
                        <div class="form-group mb-3">
                            ...
                            <textarea class="form-control @error('note') is-invalid @enderror" id="note" name="note" rows="3" placeholder="Catatan tambahan untuk pembelian ini">{{ old('note', $purchase->note) }}</textarea>
                            @error('note')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                    ...
                    <table class="table table-hover" id="products-table">
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <select class="form-select unit-select" name="units[{{ $index }}]" data-index="{{ $index }}">
                                            <option value="{{ $detail->unit_id }}" selected>{{ $detail->unit->name }}</option>
                                        </select>
                                    </td>
                                    <td>
                                        <input type="number" class="form-control quantity-input" name="quantities[{{ $index }}]" value="{{ $detail->quantity }}" min="0.01" step="0.01" required data-index="{{ $index }}">
                                    </td>
                                    <td>
                                        <div class="input-group">
                                            <span class="input-group-text">Rp</span>
                                            <input type="number" class="form-control price-input" name="prices[{{ $index }}]" value="{{ $detail->price }}" min="0" step="0.01" required data-index="{{ $index }}">
                                        </div>
                                    </td>
                                    ...
                                    <td>
                                        ...
                                        <input type="hidden" name="product_codes[]" value="{{ $detail->product->code }}">
                                        <input type="hidden" name="product_names[]" value="{{ $detail->product->name }}">
                                    </td>
                                </tr>
                            @endforeach
                        </tbody>
                        <tfoot>
                            <tr class="table-light">
                                ...
                                <th id="total-amount" class="text-primary fw-bold">{{ number_format($purchase->total_amount, 0, ',', '.') }}</th>
                                <th></th>
                            </tr>
                        </tfoot>
                    </table>
                ...
                <input type="hidden" name="total_amount" id="total-amount-input" value="{{ old('total_amount', $purchase->total_amount) }}">
                
                <div class="card mb-0 mt-4">
                    <div class="card-body">
                        <div class="d-flex justify-content-between">
                            <button type="submit" class="btn btn-primary">
                                <i class="fas fa-save me-1"></i> Perbarui Pembelian
                            </button>
                            <a href="{{ route('purchases.show', $purchase) }}" class="btn btn-outline-secondary">
...
<script>
    $(document).ready(function() {
        ...
        // Product counter
        let productCounter = {{ count($purchase->purchaseDetails) }};
