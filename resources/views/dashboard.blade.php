            ...
            <div class="card shadow mb-4">
                ...
                <div class="card-body">
                    ...
                    <div class="mt-4 text-center small">
                        @if(isset($topProductsData) && isset($topProductsData['labels']) && count($topProductsData['labels']) > 0)
                            @foreach($topProductsData['labels'] as $index => $label)
                                @if($index < 5)
                                <span class="me-2">
                                    <i class="fas fa-circle text-{{ $index == 0 ? 'primary' : ($index == 1 ? 'success' : ($index == 2 ? 'info' : ($index == 3 ? 'warning' : 'danger'))) }}"></i> {{ $label }}
                                </span>
                                @endif
                            @endforeach
                        @else
        ...
        <div class="col-12">
            <div class="card shadow mb-4">
                                ...
                                @if(isset($recentTransactions) && count($recentTransactions) > 0)
                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            ...
                                            @else
                                                <span class="badge bg-danger bg-opacity-10 text-danger">{{ ucfirst($transaction->status) }}</span>
                                            @endif
                                        </td>
                                        <td>
                                            <div class="btn-group">
                                                @if($transaction->type == 'Sale')
                                                <a href="{{ route('sales.show', $transaction->id) }}" class="btn btn-sm btn-outline-primary" data-bs-toggle="tooltip" title="Lihat Detail">
                                                    <i class="fas fa-eye"></i>
                                                </a>
                                                <a href="{{ route('sales.receipt', $transaction->id) }}" class="btn btn-sm btn-outline-success" data-bs-toggle="tooltip" title="Cetak">
                                                    <i class="fas fa-print"></i>
                                                </a>
                                                @elseif($transaction->type == 'Purchase')
                                                <a href="{{ route('purchases.show', $transaction->id) }}" class="btn btn-sm btn-outline-primary" data-bs-toggle="tooltip" title="Lihat Detail">
                                                    <i class="fas fa-eye"></i>
                                                </a>
                                                <a href="{{ route('purchases.receipt', $transaction->id) }}" class="btn btn-sm btn-outline-success" data-bs-toggle="tooltip" title="Cetak">
                                                    <i class="fas fa-print"></i>
                                                </a>
                                                @else
                ...
                <div class="card-footer bg-white">
                    <a href="{{ route('sales.index') }}" class="text-primary fw-bold">Lihat semua transaksi <i class="fas fa-arrow-right ms-1"></i></a>
                </div>
            </div>
        </div>
    </div>
                    ...
                    <div class="table-responsive">
                        <table class="table table-hover" width="100%" cellspacing="0">
                            ...
                            <tbody>
                                ...
                                @if(isset($recentStoreOrders) && count($recentStoreOrders) > 0)
                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            <div class="btn-group">
                                                ...
                                                <a href="{{ route('warehouse.store-orders.create-shipment', $order->id) }}" class="btn btn-sm btn-outline-info" data-bs-toggle="tooltip" title="Buat Pengiriman">
...
<script>
    ...
    var salesChart = new Chart(ctx, {
        type: 'line',
        data: {
            labels: {!! isset($salesChartData) && isset($salesChartData['labels']) ? json_encode($salesChartData['labels']) : json_encode(['Jan', 'Feb', 'Mar', 'Apr', 'Mei', 'Jun', 'Jul', 'Agu', 'Sep', 'Okt', 'Nov', 'Des']) !!},
            datasets: [{
                label: "Penjualan",
                lineTension: 0.3,
                ...
                pointHitRadius: 10,
                pointBorderWidth: 2,
                data: {!! isset($salesChartData) && isset($salesChartData['data']) ? json_encode($salesChartData['data']) : json_encode([5000000, 8000000, 12000000, 10000000, 15000000, 14000000, 16000000, 18000000, 21000000, 22000000, 25000000, 30000000]) !!},
            }],
        },
        options: {
    ...
    var topProductsChart = new Chart(ctx2, {
        type: 'doughnut',
        data: {
            labels: {!! isset($topProductsData) && isset($topProductsData['labels']) ? json_encode($topProductsData['labels']) : json_encode(['Produk A', 'Produk B', 'Produk C', 'Produk D','Produk E']) !!},
            datasets: [{
                data: {!! isset($topProductsData) && isset($topProductsData['data']) ? json_encode($topProductsData['data']) : json_encode([35, 25, 20, 15, 5]) !!},
    ...
    // Initialize custom datatable if needed
